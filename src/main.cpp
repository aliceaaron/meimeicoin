...
#if defined(NDEBUG)
# error "Meimeicoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Meimeicoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Meimeicoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Meimeicoin: find block wanted by given orphan block
...
// Meimeicoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Meimeicoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Meimeicoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Meimeicoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Meimeicoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Meimeicoin: track money supply and mint amount info
...
// Meimeicoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Meimeicoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Meimeicoin: compute stake entropy bit for stake modifier
    ...
    // Meimeicoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Meimeicoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Meimeicoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Meimeicoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Meimeicoin-loadblk");
